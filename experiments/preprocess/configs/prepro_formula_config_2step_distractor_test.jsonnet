...
local valid_data_config = import "./prepro_formula_config_2step_distractor_valid.jsonnet";
local train_data_config = import "./prepro_formula_config_2step_distractor_train.jsonnet";

valid_data_config + {
  config_file_path: "%s/data_configs/test_2step_distractor_config.jsonnet" % [
    CURRENT_DIR,
  ],
  output_dir: "%s/%s/raw/test_2step_distractor" % [
    DATASET_DIR,
    std.extVar("TAG"),
  ],
  number_of_data: 2000,
